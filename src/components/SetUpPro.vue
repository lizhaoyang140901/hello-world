...
export default {
      ...
      proInputInfo: {
        list: [
          ...
          },
          {
            ...
            readonly: true,
            fsneed: ["否", "是"],
            errMessage: "招标风水师的错误提示信息"
          },
          // {
          //   id: 11,
          //   listItemInfo: "预算小金额",
          //   listItemPrompt: "输入总预算小金额",
          //   listItemText: "",
          //   type: 'digit',
          //   errMessage: "输入小金额的错误提示信息",
          // },
          // {
          //   id: 12,
          //   listItemInfo: "预算大金额",
          //   listItemPrompt: "输入总预算大金额",
          //   listItemText: "",
          //   type: 'digit',
          //   errMessage: "输入大金额的错误提示信息",
          // }
        ],
        message: ""
      },
  ...
  methods: {
    ...
    setUpPro() {
      ...
      console.log(dataText.province.length);
      console.log(dataText.city.length);
      console.log(dataText.converge.length);
      // 判断必输项是否为空
      dataText.title == -1 ? this.proDescError_content()[0].innerText = "项目名称不能为空" : this.proDescError_content()[0].innerText = "";
      // 判断是否选择了区域就直接判断省或市或区的长度
      if (dataText.province.length == 0) {
        this.proDescError_content()[1].innerText = "所选地区不能为空";
      } else {
        this.proDescError_content()[1].innerText = "";
      }

      dataText.area_name == '' ? this.proDescError_content()[2].innerText = "小区名称不能为空" : this.proDescError_content()[2].innerText = "";
      dataText.address == '' ? this.proDescError_content()[3].innerText = "详细地址不能为空" : this.proDescError_content()[3].innerText = "";
      dataText.house_type == -1 ? this.proDescError_content()[4].innerText = "房屋户型不能为空" : this.proDescError_content()[4].innerText = ""; 
      dataText.acreage == '' ? this.proDescError_content()[5].innerText = "户型面积不能为空" : this.proDescError_content()[5].innerText = "";
      dataText.decoration == -1 ? this.proDescError_content()[6].innerText = "装修阶段不能为空" : this.proDescError_content()[6].innerText = "";
      dataText.decorate_style == -1 ? this.proDescError_content()[7].innerText = "装修风格不能为空" : this.proDescError_content()[7].innerText = "";
      if (dataText.phone == undefined) {
        this.proDescError_content()[8].innerText = "手机号不能为空";
      } else if (!/^1[3456789]\d{9}$/.test(dataText.phone)) {
        this.proDescError_content()[8].innerText = "输入正确的手机号";
      } else {
        this.proDescError_content()[8].innerText = '';
      }
      dataText.fsneed == -1 ? this.proDescError_content()[9].innerText = "是否招标风水师不能为空" : this.proDescError_content()[9].innerText = "";
      if (dataText.zongmoney_min > dataText.zongmoney_max) {
        this.proDescError_content()[10].innerText = "小金额须小于大金额";
        this.proDescError_content()[11].innerText = "大金额须大于小金额";
      } else {
        this.proDescError_content()[10].innerText = "";
        this.proDescError_content()[11].innerText = "";
      }
      dataText.content == '' ? this.proDescError_content()[12].innerText = "说明方案不能为空" : this.proDescError_content()[12].innerText = "";
      if (dataText.title !== -1 &&　dataText.area_name !== '' && dataText.address !== '' && dataText.house_type !== -1 && dataText.acreage !== '' && dataText.decoration !== -1 && dataText.decorate_style !== -1 && dataText.content !== '' && dataText.phone !== undefined) {
        if (!!/^1[3456789]\d{9}$/.test(dataText.phone)) {
          if (dataText.zongmoney_min < dataText.zongmoney_max) {
            this.$axios.post('https://test.sazjob.com/backapi/Project/add', dataText).then(res => {
              console.log(res);
            });
