<template>
  <div id="chart">
    ...
    <div v-show='show' class="btnTrue" @click="setUpProgress">修改</div>
  ...
  methods: {
    ...
    delImg(){
      ...
      let deleteIcconIndex = this.index,
        ...
        this.progressImgList.splice(deleteIcconIndex, 1);
    ...
    setUpProgress () {
      ...
      if (data.work_schedule == '') {
        this.proDescError_content().innerText = '进度描述不能为空';
         ...
         this.$axios.post(this.address+'/backapi/Schedule/update',  this.$qs.stringify(data), {headers: {'Authorization': this.token},withCredentials:true})
          .then(res => {
            console.log(res);
            // alert(project_id)
    ...
    getProgressInfo () {
      let dataText = {
        schedule_id: this.$route.path.split('/')[2]
      }
      this.$axios.get(this.address+'/backapi/Schedule/view?schedule_id='+this.$route.query.schedule_id)
    ...
    post(res){
        ...
        var imgs=res.work_photo.split('|');
        for(var i=0;i<=imgs.length;i++){
            // this.$set(this.fileImgList,"url",this.address+imgs[i])
            if(imgs[i]!=undefined){
                this.fileImgList.push({url:this.address+imgs[i]});
                this.progressImgList.push(imgs[i]);                  //还差删除原有图片
            }
        }
        console.log(this.progressImgList);
        ...
        this.progressMsg=res.work_schedule;
        console.log(res);
    }
