<template>
    <div id="record">
        ...
        <div class="title">
            ...
            <span>供应商送货记录</span>
            <div class="right" v-show="show" @click="router('/gyreport')">
              发布
                <!-- <b class="picAdd iconfont">&#xe624;</b> -->
                <!-- <span class="newText">新建工作报告</span> -->
            </div>
        </div>
        <van-row class="bid" v-for="bid in report" :key='bid.index' @click="router('/shDetails',bid.schedule_id,bid.id,bid.project_id)">
          <van-col span="24">
            <div class="bidText van-ellipsis">{{bid.des}} 
              <!-- <span style="color:#6678ff;float:right;" @click="router('/TenderReportNew',bid.schedule_id)" v-show="show">修改</span>  -->
            </div>
            <div class="bidTwo">
              <span>{{new Date(bid.time*1000).getFullYear()+'年'+Number(new Date(bid.time*1000).getMonth()+1)+'月'+new Date(bid.time*1000).getDate()+'日'+new Date(bid.time*1000).getHours()+':'+new Date(bid.time*1000).getMinutes()}}</span>
            </div>
          </van-col>
...
export default {
    ...
    props: {},
    methods:{
        router: function(path,schedule_id,id,project_id) {
          var abc=this.$route.path;
          var d=abc.split('/')[2];
          this.$router.push({path:path+ "/" + this.$route.query.project_id,query:{
        ...
        info(){
          this.$axios.post(this.address+'/customapi/Songhuocheck/songhuolist',this.$qs.stringify({
