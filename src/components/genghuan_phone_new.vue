<template>
  <div>
    ...
    <!-- 手机号 -->
    <van-field v-model="tel" type="number" placeholder="请输入您的新手机号" maxlength="11" />
    <!-- 验证码 -->
    ...
    <div class="zhBtn" @click="bangding">更新提交</div>
  </div>
</template>
...
export default {
	...
	methods: {
		...
		bangding(){
			...
			if (this.tel == "") {
			...
			} else if (!/(?=.*[a-zA-Z])(?=.*[0-9])[a-zA-Z0-9]{8,12}/.test(this.value_login)) {
				this.$toast({
				  message: "请按照要求输入密码",
				  icon: "chat-o"
				});
				...
				this.$axios.post('https://test.sazjob.com/customapi/User/changephonesteptwo',{
					newphone:this.tel,
