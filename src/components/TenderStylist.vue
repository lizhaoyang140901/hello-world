...
export default {
  ...
  methods: {
    ...
    afterRead(file) {
      ...
      this.$axios
        .post(this.address+"/backapi/Base/upload", formData)
        .then(res => {
          let data = res.data;
          if ((data.status = 200)) {
    ...
    setUpProgress () {
      ...
      // 判断是否输入工作进度
      if (data.des == '') {
        this.proDescError_content().innerText = '设计说明不能为空';
      } else {
          this.proDescError_content().innerText = '';
          this.$axios.post(this.address+'/backapi/Designerreport/add',  data).then(res => {
            // console.log(res);
            // 请求发送成功再让跳转
    ...
    getProgressInfo () {
      ...
      this.$axios.post(this.address+'/customapi/Fsreport/designerreportview',dataText)
    ...
    post(res){
        res=res.data.data;
        //没有数据时
        if(res==undefined&&this.userclass_id!=2){
          this.$toast.fail('设计师未提交设计图');
        }else{
  ...
  mounted () {
	  if(typeof readuserinfo === "function"){
	      this.userclass_id = readuserinfo().userclass_id;
	     }else{
