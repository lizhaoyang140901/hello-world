  ...
  <div>
    <header>
		<van-row>
			...
			<van-col class="zshejishi">修改资质</van-col>
		</van-row>
    </header>
		<div>
...
export default {
	...
	tijiao() {
		if (this.zizhi_title == "") {
		  this.$toast({
			...
			var that = this;
			this.$axios.post("https://test.sazjob.com/backapi/Zizhi/update",{
				id:this.$route.params.id,
				title:this.zizhi_title,
				img: this.touurl?this.touurl:this.tt.slice(this.tt.indexOf("com/") + 3)
			}).then(_d => {
				this.$toast({
					message: "修改成功",
					icon: "chat-o"
  ...
  mounted (){
  	  this.$axios.post('https://test.sazjob.com/customapi/Zizhi/view',{
		  id:this.$route.params.id
	  }).then(_d => {
  	  console.log(_d);
		this.zizhi_title = _d.data.data[0].title;
  		this.tt = "https://test.sazjob.com" + _d.data.data [0] .img;
  	  })
  },
};
</script>
