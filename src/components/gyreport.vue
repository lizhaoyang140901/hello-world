<template>
  <div id="chart">
    ...
    <div class="box">
      ...
      <div class="progressDesTitle">送货清单</div>
      <van-field 
        v-show='show'
        class="progressDes"
        v-model="progressMsg"
...
export default {
    ...
    setUpProgress () {
      ...
      if (data.des == '') {
        this.proDescError_content().innerText = '进度描述不能为空';
         ...
         this.$axios.post(this.address+'/customapi/Songhuocheck/songhuoadd',  this.$qs.stringify(data), {headers: {'Authorization': this.token},withCredentials:true})
          .then(res => {
            console.log(res);
            // alert(project_id)
  ...
  mounted () {
        ...
        if(typeof readuserinfo === "function"){
            this.userid = readuserinfo().userclass_id;
            this.token = readuserinfo().token;
        }else{
